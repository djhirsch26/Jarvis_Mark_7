...
RCT_EXPORT_METHOD(setRepeating:(NSString*)repeating resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
	[self prepareForPlayer:[RNSpotifyCompletion onReject:^(RNSpotifyError* error) {
		[error reject:reject];
	} onResolve:^(id unused) {
		SPTRepeatMode repeatMode = SPTRepeatOff;
		NSLog( @"Repeat Context: '%@'", repeating);
		if([repeating isEqualToString:@"context"]) {
			repeatMode = SPTRepeatContext;
		} else if([repeating isEqualToString:@"track"]) {
