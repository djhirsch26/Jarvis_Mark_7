...
export default function Draggable(props) {
  ...
  const touchable = feedback ?
  <TouchableOpacity { ...touchableProps}>{touchableContent}</TouchableOpacity> :
  <TouchableWithoutFeedback { ...touchableProps}>{touchableContent}</TouchableWithoutFeedback>

  if (xPos!=undefined || yPos!=undefined) {
    var x_fixed = xPos!=undefined ? xPos : offsetFromStart.current.x;
    var y_fixed = yPos!=undefined ? yPos : offsetFromStart.current.y;
    pan.current.setValue({x: x_fixed, y: y_fixed});
    pan.current.flattenOffset();
  }
